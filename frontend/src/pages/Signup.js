...
const Signup = () => {
  ...
  return (
    ...
    <form className="flex flex-col border relative bg-white p-5 rounded-lg h-auto w-2/6 min-w-[250px] max-w-[400px] min-h-[350px] gap-2" onSubmit={handleSubmit}>
      ...
      <button type="submit" className=' absolute bottom-2 right-2 h-7 pl-1 pr-1 border-2 rounded-lg border-teal-300 hover:bg-teal-300 hover:text-white transition:hover duration-300' disabled={isLoading}>Sign up</button>
    </form>
  </div>
  )
}
